...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete TRUSTplus database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no TRUSTplus datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old TRUSTplus datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected TRUSTplus 0.7 datadir."
    ;;
  3)
    echo "Detected TRUSTplus pre-0.8 datadir."
    ;;
  4)
    echo "Detected TRUSTplus 0.8 datadir."
    ;;
esac
