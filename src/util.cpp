...
const char * const BITCOIN_CONF_FILENAME = "trustplus.conf";
const char * const BITCOIN_PID_FILENAME = "trustplusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "trustplus" is a composite category enabling all TRUSTplus-related debug output
            if(ptrCategory->count(string("trustplus"))) {
...
#else
    const char* pszModule = "trustplus";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TRUSTplusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TRUSTplusCore
    // Mac: ~/Library/Application Support/TRUSTplusCore
    // Unix: ~/.trustpluscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TRUSTplusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TRUSTplusCore";
#else
    // Unix
    return pathRet / ".trustpluscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty trustplus.conf if it does not excist
...
//        	ReadConfigFile(mapSettingsRet, mapMultiSettingsRet);
        	...
        	LogPrintf("trustplus.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override trustplus.conf
